#!/usr/bin/env python3
"""
Corrected analyzer for Grade 3 cross-topic dependencies in Topics T25-T36
Uses correct topic mappings:
- T01: Algorithms
- T06: Events
- T07: Loops
- T08: Conditionals
...
- T11: Functions
- T16: User Interfaces
- T28: Chance & Simulations (includes random)
"""
...
def find_appropriate_dependency(all_skills, topic_prefix, max_grade='G3'):
    ...
    # For each grade level (prefer lower grades), find foundational skills
    for grade in grade_order[:max_grade_idx + 1]:
        # First pass: Look for .01 or .02 skills (foundational)
        candidates = []
        for skill_id, skill in all_skills.items():
            if skill_id.startswith(f'{topic_prefix}.{grade}.'):
                if re.search(r'\.0[12]$', skill_id):
                    candidates.append((skill_id, skill['skill_name']))

        if candidates:
            # Return the earliest one (prefer .01 over .02)
            candidates.sort()
            return candidates[0]

    # Second pass: Any skill from the topic at or below max_grade
    for grade in grade_order[:max_grade_idx + 1]:
        for skill_id, skill in all_skills.items():
            if skill_id.startswith(f'{topic_prefix}.{grade}.'):
...
def analyze_with_specific_deps(all_skills):
    ...
    for skill in grade3_skills:
        ...
        # Define cross-topic dependency checks with correct topic numbers
        checks = [
            {
            ...
            {
                'patterns': [r'\bevent\b', r'\btrigger\b', r'\bwhen clicked\b', r'\bwhen pressed\b', r'\bbroadcast\b', r'\bmessage\b', r'\bgreen flag\b', r'\bkey press\b'],
                'topic': 'T06',
                'name': 'Events'
            },
            {
                # Random is part of T28 (Chance & Simulations), only suggest for non-T28 skills
                'patterns': [r'\brandom\b', r'\bpick random\b'],
                'topic': 'T28',
                'name': 'Random/Chance'
            },
            {
            ...
            {
                'patterns': [r'\bbutton\b', r'\bwidget\b', r'\binterface\b', r'\bdisplay\b', r'\bshow.*on stage\b', r'\bask\b', r'\banswer\b', r'\bsprite\b', r'\bstage\b', r'\bmonitor\b'],
                'topic': 'T16',
...
def main():
    ...
    print(f"\n{'='*100}")
    print("GRADE 3 (T25-T36) MISSING CROSS-TOPIC DEPENDENCIES - FINAL RECOMMENDATIONS")
    print(f"{'='*100}\n")
    ...
    for rec in recommendations:
        ...
        for miss in rec['missing']:
            ...
            print(f"REASON: Uses {miss['name']} ({miss['topic']})")
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        ...
        f.write("="*100 + "\n")
        f.write("Format: Each skill lists dependencies that should be ADDED\n")
        f.write("="*100 + "\n\n")
        ...
        for rec in recommendations:
            ...
            for miss in rec['missing']:
                ...
                f.write(f"REASON: Uses {miss['name']} ({miss['topic']})\n")

            f.write("\n")

    print(f"{'='*100}")
    print(f"\nSummary:")
    print(f"  Total Grade 3 T25-T36 skills analyzed: {total_skills}")
    print(f"  Skills needing new dependencies: {len(recommendations)}")
    print(f"  Percentage: {len(recommendations)/total_skills*100:.1f}%")

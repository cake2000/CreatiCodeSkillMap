# T05 – Human‑Centered Design: K–8 Skill List (v2, IXL‑Style Microsteps)

Topic reference: `T05 Human‑Centered Design` in `domains_topics_overview.md`  
Domain: Algorithms & Design (D1) · Primary CSTA focus: ALG‑HD (with links to ALG‑IM, ALG‑AF, PRO‑PF, DAA‑DI)

This v2 version refines the original draft (`skills_T05_human_centered_design.md`) into an **IXL‑style microstep design**:

- **Role of T05:** Focused on *human‑centered design* (empathy, user needs, accessibility, iteration) and *simulation planning* (what to model, what to simplify, what data to collect) when creating or evaluating technologies and projects. It assumes core algorithm skills from **T01**, representation/tracing skills from **T02**, and project‑planning skills from **T03**, and layers user‑ and data‑centered thinking on top.  
- **Microsteps:** Each skill targets a small, explicit practice (identify needs, describe a user, spot usability issues, choose accessibility improvements, pick simulation variables, interpret feedback/data, etc.) rather than broad design bundles. Within each strand, IXL‑style microsteps move from “notice” → “categorize” → “propose one change” → “compare/explain,” with no big jumps.  

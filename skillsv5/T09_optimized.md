# T09 - Variables & Expressions (Optimized - November 2025)
# Optimizations:
# 1. Fixed all X-2 rule violations for intra-topic dependencies
# 2. Added bridging skills: G5.02.02 (string awareness), G5.09 (string operator prep), G7.09 (scope debugging)
# 3. Improved verb specificity (eliminated "Understand", improved K-2 descriptions)
# 4. Maintained ~95 skills total (added 3 new skills)
# 5. All cross-topic dependencies preserved exactly as-is

ID: T09.GK.01
Topic: T09 – Variables & Expressions
Skill: Recognize that labels can show numbers
Description: **Student task:** Look at pictures showing labels with numbers (e.g., "Score: 5", "Lives: 3", "Stars: 2"). Match each label to what it counts in the picture. _Implementation note: Picture-based matching activity with visual number-label connections. CSTA: EK‑PRO‑PF‑02._
...
ID: T09.GK.02
Topic: T09 – Variables & Expressions
Skill: Identify which label changed after clicking
Description: **Student task:** Look at two pictures: before clicking and after clicking. Circle which label changed (e.g., "Score went from 2 to 3 after clicking the star"). _Implementation note: Before/after comparison activity with click-select interaction. CSTA: EK‑PRO‑PF‑02._

Dependencies:
* T09.GK.01: Recognize that labels can show numbers
...
Description: **Student task:** Click a big button to add 1 to a counter on screen. Watch the number go up each time. Try clicking five times and count along! _Implementation note: Interactive counter with visual feedback and sound effects. CSTA: E1‑PRO‑PF‑02._

Dependencies:
* T09.GK.02: Identify which label changed after clicking
...
ID: T09.G1.02
Topic: T09 – Variables & Expressions
Skill: Use a picture-based counter to track stars collected
Description: **Student task:** In a simple picture game, drag stars into a basket and watch the counter go up. How many stars did you collect? _Implementation note: Drag-and-drop collection activity with automatic counter updates. CSTA: E1‑PRO‑PF‑02._
...
ID: T09.G2.01
Topic: T09 – Variables & Expressions
Skill: Set a starting value for a counter before the game begins
Description: **Student task:** Before the game starts, set the score to 0 using a picture card. Then play and collect items. Why do we start at 0 instead of 100? _Implementation note: Picture-based initialization activity with discussion prompt. CSTA: E2‑PRO‑PF‑02._

Dependencies:
* T09.G1.02: Use a picture-based counter to track stars collected
...
ID: T09.G2.02
Topic: T09 – Variables & Expressions
Skill: Compare a counter to a target to trigger something special
Description: **Student task:** Watch the counter go up as you collect items. When it reaches the target number (like 5), something special happens! Predict when the surprise will happen. _Implementation note: Picture-based prediction with threshold triggering and celebration animation. CSTA: E2‑PRO‑PF‑02._

Dependencies:
* T09.G2.01: Set a starting value for a counter before the game begins
...
Dependencies:
* T09.G2.02: Compare a counter to a target to trigger something special
...
ID: T09.G3.02
Topic: T09 – Variables & Expressions
Skill: Use change block to increase a variable by any amount
...
ID: T09.G3.02.01
Topic: T09 – Variables & Expressions
Skill: Use reduce block to decrease a variable by any amount
...
Description: Students use the "pick random (min) to (max)" block to set variables to random values, creating unpredictable behavior in games. They understand that the random value is different each time and falls within the specified range. Examples: random enemy spawn position, random question selection, random power-up duration.
...
ID: T09.G4.09
Topic: T09 – Variables & Expressions
Skill: Display variables in different monitor modes
Description: Students experiment with different variable monitor display modes: normal readout (shows name and value), large readout (shows only value in big text), and slider (allows interactive value adjustment). They choose appropriate display modes for different purposes: sliders for adjustable settings, large readouts for prominent scores, normal readouts for debugging.
...
ID: T09.G4.09.01
Topic: T09 – Variables & Expressions
Skill: Debug variable initialized to wrong value
Description: Students inspect a script (5-8 blocks) where a variable is initialized but to the wrong starting value. They trace through execution to recognize symptoms (e.g., score starts at 10 instead of 0, countdown starts at wrong number) and locate the incorrect initialization value. This extends G3.04.01 to more complex scripts with multiple variables.
...
ID: T09.G4.09.02
Topic: T09 – Variables & Expressions
Skill: Debug variable modified in the wrong place
Description: Students inspect a script (5-8 blocks) where a variable is changed or set in the wrong location (wrong event handler, wrong sprite, or wrong position in script). They trace execution flow to identify where the variable should be modified versus where it actually is modified. This extends G3.04.02 to multi-script programs.

Dependencies:
* T09.G4.09.01: Debug variable initialized to wrong value
...
Description: Students inspect a script (5-8 blocks) where a variable updates the correct amount but the wrong number of times (e.g., score increases by 1 ten times instead of once, or countdown doesn't decrease every iteration). They identify whether the update is inside/outside a loop incorrectly or missing a conditional guard. This extends G3.04.03 to loop and timing-related bugs.

Dependencies:
* T09.G4.09.02: Debug variable modified in the wrong place
...
ID: T09.G5.02.02
Topic: T09 – Variables & Expressions
Skill: Distinguish between number and string variable contents
Description: Students identify when variables contain numbers versus text strings, and predict how operations behave differently (e.g., "1" + "2" = "12" as string vs 1 + 2 = 3 as numbers). They recognize that user input is initially text and may need conversion for math operations. This awareness prepares for G6 string operations.
...
ID: T09.G5.09
Topic: T09 – Variables & Expressions
Skill: Apply basic string operations for text formatting
Description: Students combine join operations with simple text formatting techniques: adding spaces, line breaks, or punctuation to create well-formatted output messages. Examples: "join [Score: ] [score] [points]", creating multi-line displays. This prepares for G6's comprehensive string operator suite.
...
Dependencies:
* T09.G5.09: Apply basic string operations for text formatting
...
ID: T09.G7.01.04
Topic: T09 – Variables & Expressions
Skill: Use min and max functions in expressions
Description: Students use min() and max() functions to find smallest or largest values and to constrain variable values within bounds. Examples: "set x to max(0, min(480, x))" to keep x between 0 and 480, or "set health to max(0, health)" to prevent negative health. This is essential for game boundaries and value validation.
...
Description: Students identify and fix bugs where variables don't update as expected due to scope issues (accessing wrong sprite's variable) or timing problems (variable read before it's set). They trace variable states across sprites and event sequences to diagnose these intermediate-level bugs. This prepares for G8.04's concurrent update debugging.
...
Dependencies:
* T09.G7.01.04: Use min and max functions in expressions
* T09.G7.06: Predict behavior changes from modifying variable values
* T06.G6.01: Trace event execution paths in a multi‑event program
* T07.G6.01: Trace nested loops with variable bounds

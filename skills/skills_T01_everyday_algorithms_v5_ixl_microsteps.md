# T01 – Everyday Algorithms & Step‑by‑Step Thinking: K–8 Skill List (v5, IXL‑Style Microsteps)
...
This v5 version is designed to feel like **IXL for algorithms**, with:

- **No big jumps**: each new skill is a tiny step from the previous one.
- **Heavier coverage in coding years (3–8)**, especially for **loops** and **variables** as algorithm tools.
- A mix of **concept‑only**, **read/trace**, **write/refactor**, and **debug** skills for each idea.
- K–2 skills that remain **picture‑based and playful**, per `docs/K2_QUALITY_GUIDELINES.md`.

Skills are grouped by grade and within each grade by strands. IDs follow `T01.G<grade>.<nn>`. This file replaces v4 as the **most granular design** for T01.
...
Students find which action happens over and over in a picture‑based routine (e.g., stir, stir, stir).  
_Format:_ Highlight or MCQ. · _CSTA:_ E2‑ALG‑AF‑01.

**T01.G2.02 – Use “repeat” to describe repeated actions**  
Students choose the shorter description of repeated steps that uses “repeat ___ times.”  
...
Students rewrite a long list of repeated steps as one step with “repeat ___ times.”  
...
**T01.G2.11 – Write directions to navigate a simple grid**  
...
**T01.G2.12 – Fix maze directions that miss the goal**  
...
## Grade 3 – First Algorithms in Code (Loops & Choices in Baby Steps)
...
Students highlight which part of a script repeats the same action several times.  
_Format:_ Highlight or click region in code. · _CSTA:_ E3‑ALG‑AF‑01.

**T01.G3.04 – Predict how many times repeated blocks run**  
Students count how many times an action happens based on repeated blocks (e.g., 4× `move 10`).  
_Format:_ MCQ; auto‑graded. · _CSTA:_ E3‑ALG‑AF‑01.

**T01.G3.05 – Replace repeated blocks with a repeat loop**  
Students refactor repeated blocks into a `repeat` loop with the correct count.  
...
**T01.G3.14 – Debug a loop or if/then that doesn’t trigger correctly**  
Students fix small mistakes in loop counts or if/then conditions so a simple game behaves as described.  
_Format:_ Coding edits; auto‑graded with multiple tests. · _CSTA:_ E3‑ALG‑AF‑01, E3‑PRO‑TR‑03.

---

## Grade 4 – Loops & Variables as Algorithm Tools (Still Small Steps)
...
## Grade 5 – Representing, Comparing, and Proving Algorithms (Loops & Variables in Patterns)
...
Students match everyday‑language descriptions of algorithms to flowcharts.  
_Format:_ MCQ matching. · _CSTA:_ E5‑ALG‑AF‑01.

**T01.G5.02 – Convert a flowchart or pseudocode into code**  
Students implement simple flowcharts/pseudocode in block‑based code.  
_Format:_ Coding; auto‑graded. · _CSTA:_ E5‑ALG‑AF‑01, E5‑PRO‑PF‑01.

**T01.G5.03 – Convert a short program into pseudocode**  
Students rewrite code as structured pseudocode showing loops, if/then, and variables.  
_Format:_ Guided pseudocode; rubric/auto‑grading. · _CSTA:_ E5‑ALG‑AF‑01, E5‑ALG‑PS‑03.
...
Students reduce a long algorithm to a shorter one using loops or helper blocks without changing behavior.  
...
Students design a flowchart for a game turn (ask, check, update score, continue/stop).  
_Format:_ Flowchart design; rubric. · _CSTA:_ MS‑ALG‑AF‑01.

**T01.G6.08 – Implement code from a detailed flowchart**  
Students map each shape in a flowchart to code constructs.  
_Format:_ Coding; auto‑graded structure + tests. · _CSTA:_ MS‑ALG‑AF‑01, MS‑PRO‑PF‑01.
...
**T01.G7.5 – Design a set of edge‑case tests for an algorithm**  
...
- **K–2:** 8–12 small, playful skills per grade; focus on everyday sequences, choices, repeats, and simple tracing/debugging with pictures.
- **G3–5:** 12–14 skills per grade with many micro‑steps around:
  - Loops: find repeated blocks → count repetitions → replace with loops → trace loops → tune loop counts → apply loops in search/accumulation.
  - Variables: recognize variables → trace counters → add counters to algorithms → use variables to track game state → reason about correctness.
- **G6–8:** 8–10 skills per grade targeting:
  - Efficiency and scaling (step counts & unnecessary work),
  - Fairness and societal impacts,
  - Simulation algorithms and recursive thinking,
  - Refactoring for clarity and efficiency.

This layout is deliberately “IXL‑painful‑small”: each skill is a tiny move from the previous one and can be implemented as a short, focused activity on CreatiCode.  
Loops and variables are approached via **many small algorithmic roles** (identify, trace, modify, debug, compare) rather than a few big skills.

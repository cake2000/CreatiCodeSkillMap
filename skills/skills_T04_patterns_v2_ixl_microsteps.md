# T04 – Patterns & Reusable Solutions: K–8 Skill List (v2, IXL‑Style Microsteps)
...
- **Role of T04:** Focused on *identifying patterns* (repetition, structure, behavior) and *reusing solutions* (templates, loops, custom blocks, idioms) across problems.  
- **Microsteps:** Each skill handles one narrow pattern role (spot pattern, extend, describe rule, recognize repeated code, replace repetition with loops/blocks, adapt templates, compare pattern choices) instead of bundling multiple constructs.  
- **Uneven density:** Richer in **grades 3–6**, where loops, lists, custom blocks, and template reuse take off; lighter but meaningful foundations in K–2 and higher‑order analysis in 7–8.  
...
## Grade 1 – From Picture Patterns to Repeated Code

Strands: **1‑A Picture patterns to actions**, **1‑B Spot repetition in scripts**

### 1‑A: Picture patterns to actions

**T04.G1.01 – Match a picture pattern to a movement pattern**  
Students match a picture pattern (e.g., hop, clap, hop, clap) to sprite actions in a short animation.  
...
### 1‑B: Spot repetition in scripts

**T04.G1.03 – Find repeated blocks in a script**  
Students examine a small script and click the repeated sequence of blocks (e.g., three identical `move 10` blocks).  
_Format:_ Highlight region in code; auto‑graded by range. · _CSTA:_ E1‑ALG‑AF‑01, E1‑ALG‑PS‑03.

**T04.G1.04 – Match a repeated picture story to repeated code**  
Students match a picture story showing repeated actions with a script that repeats the same action sequence.  
_Format:_ Picture story + 2–3 scripts; MCQ. · _CSTA:_ E1‑ALG‑AF‑01.
...
Students read or see an everyday algorithm (e.g., “brush, rinse” repeated three times) and highlight the part that repeats.  
...
Students choose which of two algorithm descriptions is shorter/clearer: one with all steps written out vs one with “repeat 3 times.”  
...
Students rewrite a long everyday description by choosing a version that uses “repeat ___ times” correctly.  
...
Students see a script with copy‑pasted blocks and choose which part can be replaced by a loop.  
...
Students match a `repeat N` loop script (e.g., `repeat 4 { move 10 }`) to an animation or path with the same repeated behavior.  
...
Students adjust the `repeat` count to match a target pattern or path.  
...
- K–2: T04 builds on T01 (everyday sequences) and supports T07 (loops) and T02 (representing/tracing) conceptually.  
- G3–5: T04 patterns are closely tied to loops, conditionals, variables, and lists (T06–T10); dependencies should point to the relevant construct topics plus earlier T04 pattern recognitions.  
- G6–8: T04 emphasizes algorithmic patterns, reuse, and efficiency across projects; skills depend on prior programming topics and T04 microsteps but should not gate access to core constructs.  

# T03 – Problem Decomposition & Project Planning: K–8 Skill List (v2, IXL‑Style Microsteps)

Topic reference: `T03 Problem Decomposition & Project Planning` in `domains_topics_overview.md`  
...
- **Role of T03:** Focused on *breaking problems down*, *planning projects*, and *organizing work* (routines, programs, and CreatiCode projects). It assumes everyday algorithm familiarity from T01 and represent/tracing skills from T02.  
- **Microsteps:** Each skill targets one narrow role (identify parts, group/subtasks, plan sequence, use checklists, track progress, refactor into modules, estimate scope), rather than bundling multiple practices.  
...
Students arrange 3–4 pictures to show steps in a classroom or home routine (e.g., wash hands).  
_Format:_ Drag‑drop sequence; auto‑graded by final order. · _CSTA:_ EK‑PRO‑PM‑05.

**T03.GK.04 – Choose the missing middle step in a routine**  
Students see the first and last steps of a 3‑step routine and choose the picture that fits in the middle.  
...
**T03.G1.04 – Match steps to parts of a CreatiCode project**  
Students see a tiny CreatiCode project idea (e.g., “Cat says hello, then dances”) and match which step goes with each sprite or scene.  
_Format:_ Drag step labels onto sprites/scenes; auto‑graded by mapping. · _CSTA:_ E1‑PRO‑PM‑05.
...
Students drag subtasks into groups like “art,” “code,” “sound” for a small project.  
...
Students take 4–5 subtasks and order them into a simple plan (e.g., draw → code → test).  
...
Students see a mini‑project checklist and must mark which subtasks are done based on a short story (“We already drew sprites and added sounds.”).  
...
Students create a simple diagram showing how key screens/levels or components connect (e.g., “menu → level 1 → level 2 → game over”).  
_Format:_ Node‑and‑arrow diagram; auto‑graded by connectivity and labels. · _CSTA:_ E5‑PRO‑PM‑05.
...
Students create a diagram showing main sprites, key data (variables/lists), and message flows for a multi‑sprite game.  
...
- K–2 T03 skills depend primarily on T01 (everyday algorithms) and T02 K–2 (representing/tracing basic sequences).  
- G3–5 T03 skills depend on core coding topics (especially T06 Events and T07 Loops) and build planning capacity for gateway skills and early projects.  
- G6–8 T03 skills assume solid programming core (T06–T13) and earlier T03 decomposition habits; they should not serve as prerequisites for learning core constructs, but rather as supports for larger projects.  
- This design aligns with CSTA ALG‑PS and PRO‑PM/PRO‑TR progressions while keeping each skill small, clear, and CreatiCode‑implementable.

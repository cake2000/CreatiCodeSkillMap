# T12 – Program Organization, Style & Readability: K–8 Skill List (Draft v1)

Topic reference: `T12 Program Organization, Style & Readability` in `domains_topics_overview.md`
...
Code organization at PreK-K is about recognizing scripts, naming sprites with clear names, and understanding the purpose of simple code sequences.

### T12.GK.01 – Name a sprite clearly

- **Short name:** Choose a good name for a character
- **Description:** Students practice giving sprites meaningful names (e.g., "cat" instead of "sprite1" or "Player") to help anyone reading the project understand what each character is for. This introduces the habit of clear, readable naming.
- **Challenge format:** Concept, guided activity. A CreatiCode project shows a sprite without a meaningful name. Students are prompted to rename it from a list of descriptive options or type in a simple, clear name (e.g., "hero" vs "thing"). Auto‑grading checks that the new name is non-empty and descriptive (does not contain numbers-only or gibberish).
- **CSTA:** EK‑PRO‑PM‑05 (Articulate how one has completed a task).

### T12.GK.02 – Explain what a script does

- **Short name:** Tell what the code does
- **Description:** Students use simple language to explain what a block or short script sequence does (e.g., "the sprite jumps" or "the cat moves right"). This develops the habit of articulating code purpose, a foundation for later documentation.
- **Challenge format:** Concept, open-ended answer. A short script (2–3 blocks) is shown. Students use a sentence frame ("When ____ happens, the sprite ____") or drag-and-drop words to complete a description. Auto‑grading uses keyword matching or teacher review of structured responses.
- **CSTA:** EK‑PRO‑PM‑05.

### T12.GK.03 – Group scripts by event

- **Short name:** Organize scripts by when they run
- **Description:** Students begin to understand that different scripts in a project respond to different events (e.g., one script for "when green flag," another for "when key pressed"). This early awareness of script organization and purpose helps students recognize that code is grouped logically.
- **Challenge format:** Concept, multiple choice or drag-and-drop matching. A project shows 2–3 scripts with different event blocks. Students match each script to its trigger ("when green flag clicked," "when key 'space' pressed," etc.). Auto‑grading checks correct matches.
- **CSTA:** EK‑PRO‑PM‑05, EK‑PRO‑PM‑06.

---

## Grade 1

Grade 1 focuses on recognizing and following code organization, understanding simple comments, and naming variables and scripts with clear intent.

### T12.G1.01 – Find the main script in a project

- **Short name:** Spot the green-flag script
- **Description:** Students identify the script that runs first when the green flag is clicked, understanding that it is a key "entry point" to a program. This helps them navigate and organize code mentally.
- **Challenge format:** Concept, multiple choice or highlighting. A project shows 3–5 scripts with different event triggers (green flag, key press, etc.). Students click on or choose the script that runs when green flag is clicked. Auto‑grading checks the selection.
- **CSTA:** E1‑PRO‑PM‑04 (Explain the function of a code segment).

### T12.G1.02 – Name a variable clearly

- **Short name:** Give a variable a good name
- **Description:** Students practice creating variables with clear, descriptive names (e.g., "score" instead of "x" or "var1") so that anyone reading the code understands what the variable tracks.
- **Challenge format:** Coding, starter project. A script is provided with a variable used to track something (e.g., lives remaining). Students are prompted to create and rename the variable with a clear name before using it in the project. Auto‑grading checks that the variable name is descriptive and non-empty.
- **CSTA:** E1‑PRO‑PM‑04.

### T12.G1.03 – Explain the purpose of each script

- **Short name:** Say what each script does
- **Description:** Students write or select descriptions for each of 2–3 scripts in a project, explaining their purpose (e.g., "This script moves the sprite," "This script keeps score"). This builds the habit of documenting code intent.
- **Challenge format:** Concept, open-ended matching. A project has 2–3 scripts. Students match each script to a sentence frame or complete a simple description ("This script ____"). Auto‑grading uses sentence stem matching or keyword detection.
- **CSTA:** E1‑PRO‑PM‑04.

### T12.G1.04 – Organize code into separate scripts

- **Short name:** Put different behaviors in different scripts
- **Description:** Students are given a project with all code in one giant script and are asked to split it into two separate scripts by event (e.g., one for "when green flag" initialization and one for "when key pressed"). This introduces the idea of logical script organization.
- **Challenge format:** Coding, refactor starter. Provided: a project with multiple unrelated behaviors in a single script. Prompt: "Move the 'when key pressed' code into its own script." Auto‑grading checks that two separate scripts exist and that behaviors are correctly separated by event.
- **CSTA:** E1‑PRO‑PM‑05 (Collaborate with a partner to develop a program).

---

## Grade 2

Grade 2 introduces basic comments and refactoring code for clarity. Students document their design choices and organize projects intentionally.

### T12.G2.01 – Add a comment to explain code

- **Short name:** Write a comment
- **Description:** Students add a comment block (or text comment in a simple format) above or beside a script to explain what it does or why it was included (e.g., "# Move the sprite to the starting position"). This introduces external documentation.
- **Challenge format:** Coding, starter project. A script is provided without comments. Prompt: "Add a comment that explains what this script does." Auto‑grading checks that a comment block is present and contains at least 5 words describing the script's purpose.
- **CSTA:** E2‑PRO‑PM‑04 (Document the steps taken during program development).

### T12.G2.02 – Rename variables and sprites for clarity

- **Short name:** Rename to make code clearer
- **Description:** Students are given a completed or near-complete project with poorly named sprites or variables (e.g., "s1," "v") and are asked to rename them to match their actual purpose (e.g., "s1" → "player", "v" → "lives"). This reinforces that clear naming improves readability.
- **Challenge format:** Coding, refactor starter. Provided: a project with variables or sprite names that are cryptic or generic. Prompt: "Rename every variable and sprite to match what it does. For example, the score counter should be called 'score.'" Auto‑grading scans the project for variables/sprites and checks that names are descriptive (not single letters or generic "var1" patterns).
- **CSTA:** E2‑PRO‑PM‑04.

### T12.G2.03 – Use consistent naming style

- **Short name:** Use the same naming style everywhere
- **Description:** Students review a project and standardize variable and sprite names to follow a single naming convention (e.g., camelCase like "playerScore" or snake_case like "player_score"). This introduces the idea of consistency and style.
- **Challenge format:** Concept, code-reading and refactor hybrid. Show a project with inconsistent naming ("playerScore", "player_points", "PlayerLives"). Prompt: "Choose one style and explain why it's better for others to understand the code." Students can choose and apply a consistent style, or select the best option. Auto‑grading checks that all similar entities follow the same style.
- **CSTA:** E2‑PRO‑PM‑04.

### T12.G2.04 – Group related code together

- **Short name:** Put related blocks near each other
- **Description:** Students review a script with statements scattered throughout and reorganize them so related blocks are grouped (e.g., all initialization at the top, then movement logic, then collision checks). This introduces code organization by logical section.
- **Challenge format:** Coding, refactor starter. Provided: a script with blocks in a random or confusing order (e.g., sprite initialization mixed with collision logic). Prompt: "Reorganize the blocks so that all setup happens first, then the game loop." Auto‑grading checks code structure (initialization before loops, related blocks grouped).
- **CSTA:** E2‑PRO‑PM‑04.
